...
## Load in data, cleaned from Tabula 
...
test_social_early_labels <- sprintf(
  "<strong>%s</strong><br/>Address: %s",
  early_voting_list$polling_place, early_voting_list$map_addy) %>%
  lapply(htmltools::HTML)

social_early_labels <- sprintf(
  "<strong>%s</strong>",
  early_voting_list$polling_place) %>%
  lapply(htmltools::HTML)

social_voting_map <- leaflet() %>%
  ...
  addCircleMarkers(group = "Election Day Voting",
   ...
   addCircleMarkers(group = "Early Voting",
                    data = early_voting_list,
                    label = ~test_social_early_labels,
                    labelOptions = labelOptions(noHide = T, textsize = "6.5px", textOnly = TRUE, direction = "right"),
                    radius = 5,
                    color = "red",
                    fillOpacity = 0.6,
                    stroke = FALSE,
...
library(htmlwidgets)
saveWidget(social_voting_map, "social_voting_map.html")
