...
Filtering Data for All Years

```{r}

filter_2007 <- fec_data_all_years %>% 
  filter(report_year == 2007 | report_year == 2008 | report_year == 2009 | report_year == 2010 | report_year == 2011 | report_year == 2012 | report_year == 2013 | report_year == 2014 | report_year == 2015 | report_year == 2016 | report_year == 2017 | report_year == 2018 | report_year == 2019 | report_year == 2020 | report_year == 2020 | report_year == 2021| report_year == 2022) 


employee_student_data_2007 <- filter_2007 %>% 
  filter(!str_detect(contributor_employer, 'MEDICAL')) %>% #these are UMMS people
  filter(!str_detect(contributor_employer, 'MEDICINE')) %>% #med school people
  filter(!str_detect(contributor_employer, 'BALTIMORE')) %>% #not CP
  filter(!str_detect(contributor_employer, 'GLOBAL CAMPUS')) %>% #Not CP
  filter(!str_detect(contributor_employer, 'LAW')) %>%#not CP
  filter(!str_detect(contributor_employer, 'PHARMACY')) %>% #school of pharmacy -- not CP
  filter(!str_detect(contributor_employer, 'HOSP')) %>%  #hospital, not CP
  filter(!str_detect(contributor_employer, 'SYSTEM')) %>% #medical system people, not CP
  filter(!str_detect(contributor_employer, 'MEDIC')) %>% #school of medicine, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY COLLEGE')) %>% #UMUC, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY COL')) %>% #UMUC, not CP
  filter(!str_detect(contributor_employer, 'SCHOOL OF M')) %>% #school of medicine, not CP
  filter(!str_detect(contributor_employer, 'SOCIAL')) %>% #school of social wor, not CP
  filter(!str_detect(contributor_employer, 'NURS')) %>% #nursing school -- not CP
  filter(!str_detect(contributor_employer, 'PHARM')) %>% #school of pharmacy -- not CP
  filter(!str_detect(contributor_employer, 'MED')) %>% #med school -- not cp
  filter(!str_detect(contributor_employer, 'CAREY')) %>% #law school -- not CP
  filter(!str_detect(contributor_employer, 'SOCIA')) %>% #school of social work -- not CP
  filter(!str_detect(contributor_employer, 'SOM')) %>% #"School of Medicine" -- not CP
  filter(!str_detect(contributor_employer, 'UMBC')) %>% #UMBC
  filter(!str_detect(contributor_employer, 'PX')) %>% 
  filter(!str_detect(contributor_employer, 'BAL')) %>% #baltimore, not CP
  filter(!str_detect(contributor_employer, 'CENTER FOR ENVI')) %>% #center for envirtonmental science, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND CENTER FOR ENV.')) %>% 
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND CENTRAL MARYLAND')) %>% #idk what this is, but it's not CP
  filter(!str_detect(contributor_employer, 'DENT')) %>% #dental school, not CP
  filter(!str_detect(contributor_employer, 'SAINT JOSEPH')) %>% #not  CP
  filter(!str_detect(contributor_employer, 'CAPITAL')) %>% 
  filter(!str_detect(contributor_employer, 'SHORE')) %>% #eastern shore -- not CP
  filter(!str_detect(contributor_employer, 'RETIRED')) %>%#not current employees
  filter(!str_detect(contributor_occupation, "RETIRED")) %>% 
  filter(!str_detect(contributor_employer, 'CHESAPEAKE')) %>% #Upper Chesapeake Health -- not CP
  filter(!str_detect(contributor_employer, 'EURO')) %>% #european divisions of UMD
  filter(!str_detect(contributor_employer, 'TRAUMA')) %>% #part of UMMS
  filter(!str_detect(contributor_employer, 'PHYSICIAN')) %>% #
  filter(!str_detect(contributor_employer, 'ASIA')) %>% #not CP
  filter(!str_detect(contributor_employer, 'REHAB')) %>% #part of UMMS
  filter(!str_detect(contributor_employer, 'CHESAPEAK')) %>% 
  filter(!str_detect(contributor_employer, 'SCHOOL OF NUR')) %>% #school of nursing, not CP
  filter(!str_detect(contributor_employer, 'KING')) %>% 
  filter(!str_detect(contributor_employer, 'ALUMNI')) %>% #alumni association
  filter(!str_detect(contributor_employer, 'GREENEBAUM')) %>% #greenebaum cancer center, part of umms
  filter(!str_detect(contributor_employer, 'UNIVERSITY CITY')) %>% 
  filter(!str_detect(contributor_employer, 'ORTHAPEDIC')) %>% #part of UMMS
  filter(!str_detect(contributor_employer, 'DERMATOLOGIS')) %>% #part of UMMS
  filter(!str_detect(contributor_employer, 'UK 1964-1980')) %>% 
  filter(!str_detect(contributor_employer, 'LOYOLA')) %>% #not CP
  filter(!str_detect(contributor_employer, 'EASTERN')) %>% #eastern shore, not CP
  filter(!str_detect(contributor_employer, 'UMUC')) %>% 
  filter(!str_detect(contributor_employer, 'INSTITUTE OF H')) %>% 
  filter(!str_detect(contributor_employer, 'SURG')) %>% #surgery, not CP
  filter(!str_detect(contributor_employer, 'TRAMA')) %>% #part of UMMS
  filter(!str_detect(contributor_employer, 'UNIV. COLLEGE')) %>% #UMUC
  filter(!str_detect(contributor_employer, 'BATIMORE')) %>% #not CP
  filter(!str_detect(contributor_employer, 'METICAL')) %>% #UMMS
  filter(!str_detect(contributor_employer, 'BIOTECH')) %>% #institute of biotech is in rockville
  filter(!str_detect(contributor_employer, 'JOSEPH')) %>% #st joseph, not CP
  filter(!str_detect(contributor_employer, 'HEALTHCARE')) %>% 
  filter(!str_detect(contributor_employer, 'HARFORD')) %>% #harford memorial hospital, not CP
  filter(!str_detect(contributor_employer, 'SCHOOL OF SOCI')) %>% #school of social work, not CP
  filter(!str_detect(contributor_employer, 'UNIV. COLL')) %>% #UMUC
  filter(!str_detect(contributor_employer, 'UMROI')) %>% #rehab and ortho institute, not CP
  filter(!str_detect(contributor_employer, 'SHOR')) %>% #eastern shore, not CP
  filter(!str_detect(contributor_employer, 'UCMC')) %>% #chesapeake medical center
  filter(!str_detect(contributor_employer, 'HARDIN')) %>% 
  filter(!str_detect(contributor_employer, 'HSHSL')) %>% #Health Sciences and Human Services Library, not CP
  filter(!str_detect(contributor_employer, 'SCHOOL OF LA')) %>% #law school, not CP
  filter(!str_detect(contributor_employer, 'SSW')) %>% #school of social work, not CP
  filter(!str_detect(contributor_employer, 'CHARLES')) %>% 
  filter(!str_detect(contributor_employer, 'SCHOOL OF NU')) %>% #school of nursing, not CP
  filter(!str_detect(contributor_employer, 'ORTHO')) %>% #UMMS
  filter(!str_detect(contributor_employer, 'UNIVERSITY UC')) %>% #UMUC
  filter(!str_detect(contributor_employer, 'FPI')) %>% #faculty physicians inc, not CP
  filter(!str_detect(contributor_employer, 'UPPER CHESAP')) %>% #chesapeake health
  filter(!str_detect(contributor_employer, 'UCH')) %>% #upper chesapeake health
  filter(!str_detect(contributor_employer, 'SOL')) %>% #school of law
  filter(!str_detect(contributor_occupation, 'SURGEON')) %>% #surgeon, not CP
 filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/BC')) %>% #UMBC
   filter(!str_detect(contributor_employer, 'UNIVERISTY OF MARYLAND UNIVERSITY')) %>% 
   filter(!str_detect(contributor_last_name, 'MARQUES')) %>% #these are two people who worked for the medical school but put "University of Maryland S" on filing
   filter(!str_detect(contributor_last_name, 'COTHRAN')) %>% 
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND FACULTY PHYSICI')) %>% #this is UMD Faculty Physicians, which is in Baltimore
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/OCCUPATIONAL')) %>% #occupational health, not CP
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/ATTORNEY')) %>% #schooo of law person
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND MIDTOWN')) %>% #not cp
   filter(!str_detect(contributor_name, 'PETERS, BETH')) %>% #these are three people who put "UNIVERISTY OF MARYLAND M" and is not working in College Park
   filter(!str_detect(contributor_name, 'PIGGEE, TOMMY SR')) %>% 
   filter(!str_detect(contributor_name, 'KEATING, HEATER')) %>% 
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/DOCTOR')) %>% 
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND ME')) %>%  #this guy worked at UMD medical center
   filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/AUTOMOTIVE T')) %>% #this was some guy who lived in Oregon, so I don't think he was here
    filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND UNIVERSITY CO')) %>% 
    filter(!str_detect(contributor_employer, 'SOL')) %>% 
    filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND, SCHOOL OF PHAR')) %>% 
    filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND E')) %>% #these were Eastern Shore employees
    filter(!str_detect(contributor_name, 'FELLEN, STEVEN')) %>% #theses were people that worked at UMGC and UMUC
    filter(!str_detect(contributor_name, 'RAIDER, ALFRED')) %>% 
    filter(!str_detect(contributor_name, 'STEWART, JAMES')) %>% #umuc employee
    filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND UC')) %>% 
    filter(!str_detect(contributor_name, 'HARTIGAN, ROSEMARY')) %>%  #UMGC employee
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND SOFPHAR')) %>% 
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND COMP HEALTH')) %>% #nursing school
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/PROFESSOR/SC')) %>% #phyllis robinson -- professor at UMGC
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND UNIVESITY COLLE')) %>% 
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND & SMALL BUSINES')) %>% #this was a person associated with nursing school
  filter(!str_detect(contributor_name, 'GHEORGHIU, ILEANA')) %>% #worked at UMMS
  filter(!str_detect(contributor_name, 'KEATING, HEATHER')) %>% #worked at UMB
  filter(!str_detect(contributor_name, 'YU, LIQING')) %>% #worked at school of medicine
  filter(!str_detect(contributor_employer, 'IHV UNIVERSITY OF MARYLAND')) %>% #"institute of human virology, in baltimore
  filter(!str_detect(contributor_name, 'DEPANFILIS, DIANE')) %>% #worked at school of social work
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND PATHOLOGY ASSOC')) %>% #not in CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND M.C.')) %>% #medical center -- not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND - SJMC')) %>% #st. joseph medical center, not CP
  filter(!str_detect(contributor_employer, 'THE UNIVERSITY OF MARYLAND CANCER CENT')) %>% #not CP
  filter(!str_detect(contributor_employer, 'THE UNIVERSITY OF MARYLAND SCHOOL OF N')) %>% #school of nursing (checked individual), not CP
  filter(!str_detect(contributor_name, 'WIECHELT, SHELLY')) %>% #not in CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND MS')) %>% #medical system, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND SCHOOL OF KEDIC')) %>% 
  filter(!str_detect(contributor_name, 'HULL, BRUCE')) %>% #works at UMUC
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND-U')) %>% #person who put this works at UMBC
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND HEALTH PLANS')) %>% 
  filter(!str_detect(contributor_employer, 'HPL, UNIVERSITY OF MARYLAND')) %>%  
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND GC')) %>%  #global campus, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND, CAPITOL REGION')) %>%  
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND PRINCE GEORGE H')) %>%  #pg hospital, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND/SELF')) %>%  #these people worked outside of CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND PGHC')) %>%  #pg hospital, not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND SCHOOL OF  SOCI')) %>%  
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND CES')) %>% #center for environmental science -- not in CP 
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND-ADELPHI')) %>%  
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF UNIVERSITY OF MARYLAND')) %>%  #no record of this guy (curtis clark) working here
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND UNIVERSITY CC')) %>%  #this guy works at UMGC
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND, UPI')) %>%  
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND SCHOOL OF DESTI')) %>%  
  filter(!str_detect(contributor_employer, 'NOTRE DAME UNIVERSITY OF MARYLAND')) %>%  
  filter(!str_detect(contributor_employer, 'BWMC UNIVERSITY OF MARYLAND')) %>%
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND & SELF-EMPLOYED')) %>%  #anita tarzian was not in CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND, BC')) %>%  #baltimore county, not CP
  filter(!str_detect(contributor_name, 'WHITAKER, ALEXANDER')) %>%  #UMMS
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF MARYLAND CHILD PSYCHIATR')) %>% 
  filter(!str_detect(contributor_employer, 'INTEGRAT')) %>% #integrative health univesrity, in Laurel
  filter(!str_detect(contributor_employer, 'NOTRE DAME OF MARYLAND')) %>% #in Baltimore
  filter(!str_detect(contributor_employer, 'UNIVERSITY SUHFORM OF MARYLAND')) %>% #supposed to be USM, this person was Nancy SHapiro who is an admin there
  filter(!str_detect(contributor_employer, 'IHV')) %>% #institute of human virology
  filter(!str_detect(contributor_employer, 'SALISBURY UNIVERSITY')) %>% #not CP
  filter(!str_detect(contributor_employer, 'UNIVERSITY OF TEXAS MARYLAND ANDERSON')) %>% 
  filter(!str_detect(contributor_employer, 'TOWSON')) %>% #not CP
  filter(!str_detect(contributor_employer, 'NOTES DAME')) 
  


employee_only_data <- employee_student_data_2007 %>% 
  filter(!str_detect(contributor_occupation, "STUDENT")) %>% 
   filter(!str_detect(contributor_occupation, "RESIDENT ASSISTANT")) %>% 
   filter(!str_detect(contributor_occupation, "GRADUATE STUDENT"))   %>% 
   filter(!str_detect(contributor_occupation, "GRADUATE ASSISTANT"))   %>% 
   filter(!str_detect(contributor_occupation, "GRADUATE RESEARCH ASSISTANT"))   %>% 
   filter(!str_detect(contributor_occupation, "RESEARCH ASSISTANT"))   %>% 
   filter(!str_detect(contributor_occupation, "TEACHING ASSISTANT"))   %>% 
   filter(!str_detect(contributor_occupation, "PHD STUDENT")) %>% 
   filter(!str_detect(contributor_occupation, "RETIRED"))


  
  
fec_employers_2007 <- employee_student_data_2007 %>%       
  group_by(contributor_employer) %>% 
  summarise(sum_employer = sum(contribution_receipt_amount)) %>% 
  arrange(desc(sum_employer)) 

  


#keep foundation? alumni? #keep postdoctoral researcher? keep graduate research assistant? check on UPS entry
...
fec_data_by_year <- employee_only_data %>%  
...
fec_by_committee <- employee_only_data %>%  
  filter(report_year == 2021 | report_year == 2022) %>% 
  group_by(committee_name) %>% 
  summarise(sum_committee = sum(contribution_receipt_amount)) %>% 
  arrange(desc(sum_committee))

sum(fec_by_committee$sum_committee)
...
```{r}
student_data <- employee_student_data_2007 %>% 
  filter(str_detect(contributor_occupation, "STUDENT") | str_detect(contributor_occupation, "RESIDENT ASSISTANT") | str_detect(contributor_occupation, "GRADUATE STUDENT") | str_detect(contributor_occupation, "GRADUATE ASSISTANT") | str_detect(contributor_occupation, "GRADUATE RESEARCH  ASSISTANT") |str_detect(contributor_occupation, "RESEARCH ASSISTANT") | str_detect(contributor_occupation, "TEACHING ASSISTANT") | str_detect(contributor_occupation, "PHD STUDENT"))

student_data_by_year <- student_data %>% 
  group_by(report_year) %>% 
  summarise(student_year = sum(contribution_receipt_amount))


employee_student_data_2007 %>% 
...
data_2022 <- employee_student_data_2007 %>% 
  filter(report_year == 2022)

```

Data, 2021- 2022 Cycle

```{r}

data_2021_2022 <- employee_student_data_2007 %>% 
  filter(report_year == 2021 | report_year == 2022 )
...
act_blue <- employee_student_data_2007 %>% 
  filter(committee_name =="ACTBLUE") %>% 
  filter(report_year == 2021| report_year == 2022)

sum(act_blue$contribution_receipt_amount): 277516.7

win_red <- employee_student_data_2007 %>% 
  filter(committee_name =="WINRED") %>% 
  filter(report_year == 2021| report_year == 2022)

sum(win_red$contribution_receipt_amount): 2331


```

Committee's Party Affiliations
...
fec_by_committee <- fec_by_committee %>% 
...
sum_party <- fec_by_committee %>% 
  group_by(party) %>% 
  summarise(sum_party = sum(sum_committee)) %>% 
...
top_ten <- data_2021_2022 %>% 
  group_by(committee_name) %>% 
  filter(recipient_committee_type == "H" | recipient_committee_type == "S" | recipient_committee_type == "P") %>% #this filter was only for presidential, house, or senate affiliated committees
  summarise(sum_committee = sum(contribution_receipt_amount)) %>% 
  arrange(desc(sum_committee)) %>% 
  slice_max(sum_committee, n = 10)



top_ten %>% 
  ggplot() +
  geom_bar(aes(x=reorder(committee_name, sum_committee), weight = sum_committee, fill=sum_committee)) +
  labs(title = "Top candidate recipients of UMD employee donations, 2021-2022", 
       subtitle = "Raphael Warnock, Cheri Beasley top candidate recipients in 2021 and 2022",
...
employee_only_data$date <- sapply(strsplit(as.character(employee_only_data$contribution_receipt_date), " "), "[", 1)
employee_only_data$time <- sapply(strsplit(as.character(employee_only_data$contribution_receipt_date), " "), "[", 2)

donations_by_day <- employee_only_data %>% 
  select(contribution_receipt_amount, date) %>% 
  filter(contribution_receipt_amount > 0)
donations_by_day <- donations_by_day %>% 
  format(donations_by_day$month_year, "%Y-%m-%d")



donations_by_day$month_year <- lubridate::mdy(donations_by_day$date)
donations_by_day$month_year <- format(as.Date(donations_by_day$month_year), "%Y-%m-1")


donations_by_month <- donations_by_day %>% 
...
donations_by_month$MonthAbb <- mymonths[ donations_by_month$month ]





write_xlsx(donations_by_month, "/Users/ericneugeboren/Documents/Data Stories/monthly_donations.xlsx")
